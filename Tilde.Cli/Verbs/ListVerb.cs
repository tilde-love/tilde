...
namespace Tilde.Cli.Verbs
{
    ...
    public class ListVerb : Verb
    {
        ...
        public static IEnumerable<Example> Examples 
        {
            get 
            {
                yield return new Example("List projects", new ListVerb { ItemTypes = ListableItemTypes.Projects, ServerUri = new Uri("http://localhost:5000", UriKind.RelativeOrAbsolute) });
                yield return new Example("List files in a project", new ListVerb { ItemTypes = ListableItemTypes.Files, Project = "PROJECT", ServerUri = new Uri("http://localhost:5000", UriKind.RelativeOrAbsolute) });
                yield return new Example("List controls in a project", new ListVerb { ItemTypes = ListableItemTypes.Controls, Project = "PROJECT", ServerUri = new Uri("http://localhost:5000", UriKind.RelativeOrAbsolute) });
            }
        }
                    ...
                    case ListableItemTypes.Controls:
                    {
                        Tuple<HttpStatusCode, ProjectResult> responseTuple =
                            GetResponse<ProjectResult>(
                                    "GET",
                                    new Uri(opts.ServerUri, new Uri($"api/1.0/Projects/{opts.Project}", UriKind.Relative))
                                )
                                .Result;
                        ...
                        switch (responseTuple.Item1)
                        {
                            case HttpStatusCode.OK when opts.ItemTypes == ListableItemTypes.Files:
                                items = responseTuple.Item2.Files.Select(u => u.ToString())
                                    .ToArray();
                                break;
                            ...
                            case HttpStatusCode.OK: items = new string[0];
                                break;
